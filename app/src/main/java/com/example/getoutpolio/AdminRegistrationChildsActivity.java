...
public class AdminRegistrationChildsActivity extends AppCompatActivity {
    private ImageView uploadImage;
    private EditText objid, objName,objFatherName,objFatherCNIC,objChildAge,objAddress;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_admin_registration_childs);

        uploadImage=findViewById(R.id.team_image_Id);
        objid=findViewById(R.id.child_id);
    ...
    {
        if(isImageSelected &&!objid.getText().toString().isEmpty()&& !objName.getText().toString().isEmpty()&& !objFatherName.getText().toString().isEmpty()&& !objFatherCNIC.getText().toString().isEmpty()&& !objChildAge.getText().toString().isEmpty()&& !objAddress.getText().toString().isEmpty() )
        {
            ...
            objectUploadTask.continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() {
                ...
                public void onComplete(@NonNull Task<Uri> task) {
                    if(task.isSuccessful())
                    {
                        ...
                        objectFirebaseFirestore.collection("areas").document("Samnabad Town").collection("blocks").document("block A").collection("streets").document("Street#01").collection("houses").document("House#02").collection(COLLECTION_NAME)
                                .document(objid.getText().toString())
                                .set(objectMap)
                                .addOnSuccessListener(new OnSuccessListener<Void>() {
                                    ...
                                    public void onSuccess(Void aVoid) {
                                        ...
                                        objFatherCNIC.setText("");
                                        objChildAge.setText("");
                                        objAddress.setText("");


                                        Toast.makeText(AdminRegistrationChildsActivity.this, "Child add successfully", Toast.LENGTH_SHORT).show();
                                    }
                                })
                                .addOnFailureListener(new OnFailureListener() {
                                    ...
                                    public void onFailure(@NonNull Exception e) {
                                        ...
                                        Toast.makeText(AdminRegistrationChildsActivity.this, "Fails to upload image:"+e.getMessage(), Toast.LENGTH_SHORT).show();
                                    }
                                });
                    }
                }
